...
export function middleware(request: NextRequest) {
    ...
    if (request.nextUrl.pathname.startsWith("/login")) { // "/login"
        const response: NextResponse = NextResponse.next();
        return response;
    }
    if (request.nextUrl.pathname.startsWith("/register")) { // "/register"
        const response: NextResponse = NextResponse.next();
        return response;
    }
    if (request.nextUrl.pathname.startsWith("/") && request.nextUrl.pathname.endsWith("/")) { // "/"
        const response: NextResponse = NextResponse.next();
        return response;
    }
